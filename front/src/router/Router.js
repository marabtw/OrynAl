...
const Router = () => {
  ...
  const getInitialRoute = (role) => {
    if (role === "admin") {
      return `${removeWildcard(ROUTERS.Management.root)}${ROUTERS.Management.allRestaurants}`
    } else if (role === "owner") {
      return `${removeWildcard(ROUTERS.Restaurant.root)}${ROUTERS.Restaurant.myRestaurants}`
    } else {
      return ROUTERS.Home
    }
  }
  ...
  return (
		...
		<Route path="*" element={<h2 className="flex justify-center items-center">Ресурс не найден</h2>} />
	</Routes>
  )
}
