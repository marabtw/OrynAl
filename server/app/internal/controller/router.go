...
func (s *Server) setupAdminRoutes(g *echo.Group) {
	...
	admin.POST("/restaurants/services", s.handler.Admin.CreateService)
	admin.DELETE("/restaurants/services/:id", s.handler.Admin.DeleteService)
	admin.PUT("/restaurants/services/:id", s.handler.Admin.UpdateService)
	admin.GET("/restaurants/services", s.handler.Restaurant.GetServices)
...
func (s *Server) setupRestaurantRoutes(g *echo.Group) {
	...
	// restaurant.POST("/:id/photos", s.handler.Restaurant.) 
	//restaurant.GET("/favorite", s.handler.Restaurant.FavoriteRestaurants)
	restaurant.POST("/:id/reviews", s.handler.Reviews.CreateReview, s.jwt.ValidateUser)
